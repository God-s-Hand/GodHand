// Please read this comments.
// The Basic MPU-6050 Original code was written by Krodal but the advanced one that is about complimentary filter & processing code is written by Debra who run a blog "geekmomprojects"
// Team1 : Godhand has slightly changed the code to use our purpose. We just added some code in loop function and setup function.
...
float         muscle_value; // Value from the myoware sensor
int           activated;

enum hand_direction { up, down, left, right};
...
void loop()
{
  ...
  int sensorValue = analogRead(A0); // this is muscle sensor's value.
  float voltage = sensorValue * (5.0 / 1023.0); // convert raw value to voltage that fit in 5V.
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
 ...
 /*Serial.print(F("DEL:"));              //http://www.geekmomprojects.com/gyroscopes-and-accelerometers-on-a-chip//Delta T
  Serial.print(dt, DEC);
  ...
  Serial.println();*/
// if you want to run a processing program that show graphical conceptual rectangular, commenting below code and uncommenting above comment

  /* this is our part : Team 1 */
  Serial.print(voltage);
  Serial.print(F("#")); // Parsing token;
...
// Parsing Every command following the value filtered by complimentary expression.
  if ( activated == 3 && angle_x <= 100 && angle_x > 20){
    Serial.println("Left#");
    }
  else if ( activated == 3 && angle_x <= 20 && angle_x >= -20 && angle_y < 100 && angle_y >= 20) {
    Serial.println("Down#");
    }
  else if ( activated == 3 && angle_x <= 20 && angle_x >= -20 && angle_y < 0 && angle_y >= -100) {
    Serial.println("Up#");
    }
  else if ( activated == 3 && angle_x < -30 && angle_x >= -100) {
